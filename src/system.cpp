...
void System::draw() {
	...
	for (auto &entity : entities) {
		...
		if (hasComponent<LabelComponent>(entity)) {
			...
			textPos.y =
				rect.y + Lerp(0.0f, rect.height - textSize.y, (static_cast<float>(label.verticalAlignment) * 0.5f));
		...
		if (hasComponent<TextAreaComponent>(entity)) {
			...
			for (int i = 0; i < textArea.text.size(); i++) {
				...
				DrawTextPro(Game::getUi().getFont(), TextSubtext(textArea.text.c_str(), i, 1), charPos, Vector2{0, 0},
							0.0f, 13 * 3, 1, BLACK);
		...
		if (hasComponent<ImageRectComponent>(entity)) {
			...
			DrawTexturePro(imageRect.texture,
						   Rectangle{0, 0, static_cast<float>(imageRect.texture.width),
									 static_cast<float>(imageRect.texture.height)},
